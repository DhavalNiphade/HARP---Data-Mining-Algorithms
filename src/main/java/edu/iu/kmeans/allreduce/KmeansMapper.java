	...
	protected void mapCollective( KeyValReader reader, Context context) throws IOException, InterruptedException {
	 ...
	 private void computation(Table<DoubleArray> cenTable, Table<DoubleArray> previousCenTable,ArrayList<DoubleArray> dataPoints){
	  ...
	  private void runKmeans(List<String> fileNames, Configuration conf, Context context) throws IOException {
		  ...
		  /*int batchSize = 100;
		  int offset = batchSize*iter;
		  *//*Custom FUNCTION*//*
		  ArrayList<DoubleArray> miniBatch = new ArrayList<DoubleArray>();
		  for(int i=offset ; i < (offset+batchSize) ; i++){
		  	miniBatch.add(dataPoints.get(i));
		  }

		  System.out.println("Mini Batch Size : " + miniBatch);*/

		  ArrayList<DoubleArray> miniBatch = new ArrayList<DoubleArray>();

		  Table<DoubleArray> previousCenTable =  null;

		  int Batches=100000/numMappers;
		  ...
		  for(int iter=0; iter < iteration; iter++){
			  ...
			  //Generate Random points from the Data Points array list.
			  for(int randGenerator=0 ; randGenerator < Batches ; randGenerator++){
				  Random rand = new Random();

				  //Insert into miniBatch arrayList the newly generated dataPoints
				  //TODO : check if random points actually work for large datasets
				  miniBatch.add(dataPoints.get(rand.nextInt(dataPoints.size())));
			  }

			  System.out.println("Mini Batch Size : " + miniBatch.size());
			  System.out.println("Mini Batch Contents : " + miniBatch);
			  ...
			  computation(cenTable, previousCenTable, miniBatch);
	  ...
	  private void calculateCentroids( Table<DoubleArray> cenTable, Table<DoubleArray> exCenTable){
		  ...
		  //Take the newest centroid table and create a Partition Array of it
		  //Uses Partition from HARPs library
		  for(Partition<DoubleArray> currPartitions : cenTable.getPartitions()){

		  	//Fetch partitions of former table using the new partition ID
		  	Partition<DoubleArray> exPartition = exCenTable.getPartition(currPartitions.id());

		  	//Modify the partition size to accommodate former and new partition values
		  	currPartitions.get().get()[vectorSize] = currPartitions.get().get()[vectorSize] + exPartition.get().get()[vectorSize];


		  	//For each partition in new partitions, execute steps from algorithm 10:12
		  	for(int i=0;i<vectorSize;i++){

		  		currPartitions.get().get()[i] = (exPartition.get().get()[i]*exPartition.get().get()[vectorSize]+currPartitions.get().get()[i])/currPartitions.get().get()[vectorSize];
			}
		  }
